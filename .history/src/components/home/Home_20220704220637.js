...
const Home = () => {
  ...
  const handleOnPlayPauseClickChill = (bool = false) => {
    dispatch(toggleSlice.actions.isChillToggleHome(!isChillPlay));
  };

  const handleOnPlayPauseClickSleepy = (bool = false) => {
    dispatch(toggleSlice.actions.sleepyToggleHome(bool));
  };
    ...
    <HomeStyles>
      ...
      {isSleepyPlay && (
        <AudioPlayer
          tracks={sleepy}
          play={isSleepy}
          handleOnPlayPauseClick={handleOnPlayPauseClickSleepy}
        ></AudioPlayer>
      )}
    </HomeStyles>
  );
