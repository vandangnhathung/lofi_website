...
const Home = () => {
  ...
  const handleDayRain = async () => {
    if (dayRain === false && dayRainDashboard === true && volumeRainNum > 0) {
      await dispatch(
  ...
  const handleKeyboard = () => {
    if (
      keyboardSound === false &&
      keyboardDashBoard === true &&
      volumeKeyboardNum > 0
    ) {
      dispatch(toggleSlice.actions.keyboardDashBoardToggleHome(!keyboardSound));
      dispatch(volumeSlice.actions.volumeKeyboardAdjust(0));
    } else {
      dispatch(toggleSlice.actions.cityTrafficToggleHome(!keyboardSound));
    }
  };
