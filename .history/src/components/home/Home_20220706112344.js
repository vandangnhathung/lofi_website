...
const Home = () => {
  ...
  const isDayRain = useSelector((state) => state.toggle.rainDayToggle);
  const enterStore = useSelector((state) => state.toggle.enterStoreToggle);
  const [dayRain, setDayRain] = useState(false);
  ...
  const handleDayRain = () => {
    // setDayRain(!dayRain);
    dispatch(toggleSlice.actions.rainDayToggleHome(!isDayRain));
  };
  // useEffect(() => {
  //   if (volumeRainNum > 0) {
  //     dispatch(toggleSlice.actions.rainDayToggleHome(true));
  //     console.log("Oh shit");
  //   } else if (volumeRainNum === 0) {
