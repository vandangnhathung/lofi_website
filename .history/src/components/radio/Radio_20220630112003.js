...
const Radio = ({
  ...
  return (
    <label className="absolute cursor-pointer w-[200px] h-[72px] group flex justify-center">
      ...
      <div className="absolute top-[50%] hidden  p-3 px-6 text-white capitalize rounded-lg group-hover:animate-scale  bg-blackBackground whitespace-nowrap hover:text-opacity-30 transition-all duration-200">
        <p className="hidden group-hover:block">{text}</p>
        {dayRain ? <Slider></Slider> : ""}
      </div>
    </label>
  );
};
