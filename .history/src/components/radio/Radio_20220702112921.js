...
const Radio = ({
  ...
  return (
    <label className="absolute cursor-pointer w-[200px] h-[72px] group flex justify-center">
      ...
      <div className="absolute top-[50%] hidden p-3 px-6 text-white capitalize rounded-lg group-hover:animate-scale group-hover:height-auto  bg-blackBackground whitespace-nowrap hover:text-opacity-30 transition-all duration-200 text-center">
        <p onClick={onClick}>{text}</p>
        <div className="transition-all duration-200 group-hover:height-auto">
          {" "}
          {dayRain ? (
            <Box sx={{ width: 100 }}>
              <Stack
