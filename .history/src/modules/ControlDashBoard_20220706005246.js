...
const ControlDashBoard = ({ openControlAdjust, controlBoardWidth }) => {
  ...
  const handleChangeVolumeCityRain = async (e) => {
    if (volumeCityRainNum > 0 && !night) {
      await dispatch(toggleSlice.actions.rainDayToggleHome());
    } else if (volumeCityRainNum > 0 && night) {
      await dispatch(toggleSlice.actions.rainNightToggleHome());
    }
